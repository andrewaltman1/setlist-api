...
const getAllVenues = async (req: Request) => {
  return await pool.query(
    `SELECT venues.id as "venueId", name as "venueName", city, state, country, ST_AsGeoJSON(geom) AS geometry, COUNT(*) as "total" FROM venues join shows on venues.id = venue_id group by venues.id order by "total" DESC`,
  );
};
